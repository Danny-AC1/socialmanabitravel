...
type Tab = 'home' | 'explore' | 'search' | 'messages' | 'profile';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (saved === 'home' || saved === 'explore' || saved === 'search' || saved === 'messages' || saved === 'profile') {
        return saved as Tab;
      }
    }
    return 'home';
  ...
  const getPageTitle = () => {
      switch(activeTab) {
          ...
          case 'messages': return 'Mensajes | Ecuador Travel';
  ...
  const performNearbySearch = async (query: string) => {
      // SOLO buscamos externamente si tenemos la ubicación del usuario
      if (!userLocation) {
  ...
  const handleAddPhotoToDestination = async (image: string) => { 
      ...
      await StorageService.addPhotoToDestinationGallery( selectedDestination.id, selectedDestination.gallery, image, user.id ); 
  };
  ...
  return (
    <div className="min-h-screen bg-stone-50 pb-20 md:pb-24 font-sans">
      ...
      <nav className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-stone-200 px-4 py-3 shadow-sm">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex flex-1 max-w-md mx-8 relative">
            <input 
                type="text" 
                placeholder={userLocation ? "Buscar lugares a tu alrededor (30km)..." : "Buscar en la app..."}
          ...
          <div className="hidden md:flex space-x-6 text-stone-500 font-medium items-center">
             <button onClick={() => navigateToTab('home')} className={`hover:text-cyan-700 transition-colors ${activeTab === 'home' ? 'text-cyan-700' : ''}`} title="Inicio"><Home size={24} /></button>
             <button onClick={() => navigateToTab('explore')} className={`hover:text-cyan-700 transition-colors ${activeTab === 'explore' ? 'text-cyan-700' : ''}`} title="Explorar"><Compass size={24} /></button>
             <button onClick={() => handleOpenChat()} className={`hover:text-cyan-700 transition-colors relative ${activeTab === 'messages' ? 'text-cyan-700' : ''}`} title="Mensajes">
             ...
             <button onClick={() => openModal(setIsNotificationsOpen)} className="hover:text-cyan-700 transition-colors relative" title="Notificaciones">
                <Bell size={24} />
             ...
             {isAdminUser && (
                <button onClick={() => openModal(setIsAdminUsersModalOpen)} className="hover:text-cyan-700 transition-colors" title="Usuarios"><Users size={24} /></button>
             )}
             <button onClick={() => openModal(setIsSuggestionsModalOpen)} className="relative hover:text-cyan-700 transition-colors" title="Sugerencias">
                 <Lightbulb size={24} />
                 {isAdminUser && unreadSuggestionsCount > 0 && (
                     <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-4 h-4 flex items-center justify-center rounded-full font-bold border-2 border-white">{unreadSuggestionsCount}</span>
                 )}
             </button>
          ...
          <div className="flex md:hidden items-center gap-4 text-stone-600">
             {isAdminUser && <button onClick={() => openModal(setIsAdminUsersModalOpen)} className="text-cyan-600"><Users size={22} /></button>}
             <button onClick={() => openModal(setIsSuggestionsModalOpen)} className="text-cyan-600 relative">
                 <Lightbulb size={22} />
                 {isAdminUser && unreadSuggestionsCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-3 h-3 rounded-full border border-white"></span>}
             </button>
             <button onClick={() => handleOpenChat()} className={`relative ${activeTab === 'messages' ? 'text-cyan-600' : ''}`}>
                <MessageCircle size={24} />
      ...
      <div className="max-w-4xl mx-auto pt-4 px-4 md:px-0">
            ...
            <div>
               ...
               <div className="grid md:grid-cols-3 gap-8">
                  <div className="md:col-span-2 space-y-6">
                     ...
                     {!searchQuery && <HeroSection destination={featuredDestination} onGuideClick={handleOpenGuide} />}
                              ...
                              {recommendedDestinations.map(d => (
                                 <div key={d.id} onClick={() => handleOpenGuide(d.name)} className="flex items-center gap-3 p-2 hover:bg-stone-50 rounded-xl cursor-pointer transition-colors">
                                    ...
                                    <div className="flex-1 min-w-0">
                                       ...
                                       {userLocation && d.coordinates && <span className="text-[9px] text-emerald-600 font-bold block mt-0.5">A {calculateDistance(userLocation.lat, userLocation.lng, d.coordinates.latitude, d.coordinates.longitude).toFixed(1)} km</span>}
                                    </div>
                                 </div>
                              ))}
                           </div>
                ...
                <div className="md:col-span-2 flex flex-col gap-4 mb-4 sticky top-[60px] md:top-[76px] z-20 bg-stone-50/95 backdrop-blur-md py-4 -mx-4 px-4 md:mx-0 md:px-0 border-b border-stone-200/50 shadow-sm transition-all">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2 flex-wrap justify-end">
                          <button onClick={handleNearbySearch} className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold py-2 px-3 rounded-full flex items-center gap-2 shadow-md transition-colors w-fit animate-in fade-in slide-in-from-right-2"> <Navigation size={16} /> ¿Qué hay cerca? </button>
                          <button onClick={() => openModal(setIsItineraryModalOpen)} className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white text-xs font-bold py-2 px-3 rounded-full flex items-center gap-2 shadow-md transition-colors w-fit animate-in fade-in slide-in-from-right-4"> <Sparkles size={16} /> Planificar Viaje </button>
        ...
        {activeTab === 'messages' && (
           <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-stone-100 h-[calc(100vh-180px)] md:h-[75vh] flex flex-col animate-in fade-in duration-300">
              <ChatModal isOpen={true} onClose={() => navigateToTab('home')} currentUser={user} allUsers={allUsers} initialChatId={initialChatId} isIntegrated={true} />
           </div>
        )}
        ...
        {activeTab === 'profile' && (
             (() => {
                ...
                let targetUser = allUsers.find(u => u.id === targetId) || user;
                ...
                const visibleGroups = isMe ? travelGroups.filter(g => g.adminId === targetUser!.id) : travelGroups.filter(g => g.adminId === targetUser!.id && !g.isPrivate);
                const totalPoints = targetUser.points || 0;
                const currentLevel = getUserLevel(totalPoints);
                const nextLevel = getNextLevel(totalPoints);
                const progressPercent = nextLevel ? ((totalPoints - currentLevel.minPoints) / (nextLevel.minPoints - currentLevel.minPoints)) * 100 : 100;
                ...
                return (
                  <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-stone-100 min-h-[500px] pb-20">
                    ...
                    <div className="px-6 pb-6 relative">
                      ...
                      <div className="mb-4">
                        <h2 className="text-2xl font-bold text-stone-800 flex items-center gap-2"> {targetUser!.name} {isMe && <Edit3 size={16} className="text-stone-400 cursor-pointer" onClick={() => { setIsEditingName(true); setNewName(user.name); }} />} </h2>
                        {isMe && isEditingName && <div className="flex gap-2 my-2"> <input className="border rounded px-2 py-1 text-sm" value={newName} onChange={e => setNewName(e.target.value)} /> <button onClick={handleUpdateName} className="text-green-600 text-xs font-bold">OK</button> </div>}
                        <p className="text-stone-500 mb-2">{targetUser!.bio}</p>
                        <div className="bg-stone-50 rounded-xl p-3 border border-stone-100 mb-2">
                            <div className="flex justify-between items-center mb-1"> <span className={`text-xs font-black uppercase tracking-wider flex items-center gap-1 ${currentLevel.color}`}> {currentLevel.icon} {currentLevel.name} </span> <span className="text-[10px] text-stone-400 font-bold">{totalPoints} XP</span> </div>
                            <div className="h-2 w-full bg-stone-200 rounded-full overflow-hidden"> <div className="h-full bg-gradient-to-r from-cyan-400 to-blue-500" style={{ width: `${progressPercent}%` }}></div> </div>
                            {nextLevel ? <p className="text-[10px] text-stone-400 mt-1 text-right"> Faltan {nextLevel.minPoints - totalPoints} XP para {nextLevel.name} </p> : <p className="text-[10px] text-amber-500 mt-1 text-right font-bold">¡Nivel Máximo!</p>}
                        </div>
                        {isMe && <button onClick={() => openModal(setIsTravelGroupsOpen)} className="w-full mt-3 bg-orange-500 hover:bg-orange-600 text-white py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-md transition-colors animate-in fade-in slide-in-from-top-2"> <Users size={18} /> Mis Grupos de Viaje </button>}
                      </div>
                      ...
                      <div className="flex gap-4 border-b border-stone-100 mb-4 overflow-x-auto no-scrollbar">
                          <button onClick={() => setProfileSubTab('posts')} className={`pb-2 text-sm font-bold flex items-center gap-2 whitespace-nowrap ${profileSubTab === 'posts' ? 'border-b-2 border-cyan-600 text-cyan-700' : 'text-stone-400'}`}> <LayoutGrid size={16} /> Mis Fotos </button>
                          <button onClick={() => setProfileSubTab('groups')} className={`pb-2 text-sm font-bold flex items-center gap-2 whitespace-nowrap ${profileSubTab === 'groups' ? 'border-b-2 border-cyan-600 text-cyan-700' : 'text-stone-400'}`}> <Users size={16} /> Grupos ({visibleGroups.length}) </button>
                          <button onClick={() => setProfileSubTab('contributions')} className={`pb-2 text-sm font-bold flex items-center gap-2 whitespace-nowrap ${profileSubTab === 'contributions' ? 'border-b-2 border-cyan-600 text-cyan-700' : 'text-stone-400'}`}> <Award size={16} /> Aportes </button>
                          <button onClick={() => setProfileSubTab('badges')} className={`pb-2 text-sm font-bold flex items-center gap-2 whitespace-nowrap ${profileSubTab === 'badges' ? 'border-b-2 border-cyan-600 text-cyan-700' : 'text-stone-400'}`}> <Trophy size={16} /> Insignias </button>
                          {isMe && <button onClick={() => setProfileSubTab('map')} className={`pb-2 text-sm font-bold flex items-center gap-2 whitespace-nowrap ${profileSubTab === 'map' ? 'border-b-2 border-cyan-600 text-cyan-700' : 'text-stone-400'}`}> <MapIcon size={16} /> Mapa de Vida </button>}
                      </div>
                      {profileSubTab === 'posts' && <div className="grid grid-cols-3 gap-1"> {userPosts.length > 0 ? userPosts.map(post => <div key={post.id} className="aspect-square relative cursor-pointer bg-gray-100" onClick={() => handleViewPost(post)}> {post.mediaType === 'video' ? <video src={post.imageUrl} className="w-full h-full object-cover" /> : <img src={post.imageUrl} className="w-full h-full object-cover" />} </div>) : <div className="col-span-3 py-10 text-center text-stone-400 text-sm">No hay publicaciones.</div>} </div>}
                      {profileSubTab === 'groups' && <div className="space-y-3"> {visibleGroups.length > 0 ? visibleGroups.map(group => <div key={group.id} onClick={() => handleOpenGroup(group.id)} className="flex items-center gap-3 p-3 border rounded-xl cursor-pointer hover:bg-stone-50 bg-white"> <img src={group.imageUrl} className="w-16 h-16 rounded-lg object-cover" /> <div className="flex-1"> <div className="flex justify-between items-start"> <h4 className="font-bold text-stone-800 text-sm">{group.name}</h4> {group.isPrivate && <Lock size={12} className="text-stone-400"/>} </div> <p className="text-xs text-stone-500 line-clamp-1">{group.description}</p> <span className="text-[10px] text-stone-400 font-bold mt-1 block"> {(group.members ? Object.keys(group.members).length : 0)} miembros </span> </div> </div>) : <div className="py-10 text-center text-stone-400 text-sm bg-stone-50 rounded-xl border border-dashed"> {isMe ? 'No has creado ningún grupo aún.' : 'Este usuario no tiene grupos públicos.'} </div>} </div>}
                      {profileSubTab === 'contributions' && <div className="space-y-3"> {userContributions.length > 0 ? userContributions.map(dest => <div key={dest.id} onClick={() => handleOpenGuide(dest.name)} className="flex items-center gap-3 p-3 border rounded-xl cursor-pointer hover:bg-stone-50"> <img src={dest.imageUrl} className="w-16 h-16 rounded-lg object-cover" /> <div> <h4 className="font-bold text-stone-800 text-sm">{dest.name}</h4> <span className="text-xs text-stone-500 flex items-center gap-1"><MapPin size={10} /> {dest.location}</span> </div> </div>) : <div className="py-10 text-center text-stone-400 text-sm"> Aún no has agregado destinos turísticos a la guía. </div>} </div>}
                      {profileSubTab === 'badges' && <div className="grid grid-cols-3 md:grid-cols-4 gap-4"> {BADGES.map((badge) => { const isUnlocked = userBadges.some(b => b.id === badge.id); return <div key={badge.id} className={`flex flex-col items-center text-center p-3 rounded-xl border ${isUnlocked ? 'bg-yellow-50 border-yellow-200' : 'bg-gray-50 border-gray-100 opacity-60 grayscale'}`}> <div className="text-3xl mb-2">{badge.icon}</div> <h4 className={`text-xs font-bold mb-1 ${isUnlocked ? 'text-gray-800' : 'text-gray-400'}`}>{badge.name}</h4> <p className="text-[9px] text-gray-500 leading-tight">{badge.description}</p> {isUnlocked && <CheckCircle className="text-green-500 mt-2" size={12} />} </div>; })} </div>}
                      {profileSubTab === 'map' && <div className="animate-in fade-in zoom-in duration-300"> <div className="bg-cyan-50 p-3 rounded-xl mb-4 text-xs text-cyan-800 flex items-center gap-2 border border-cyan-100"> <MapIcon size={16} /> <p>Este mapa se genera automáticamente con las ubicaciones de tus fotos.</p> </div> <LifeMap posts={userPosts} /> </div>}
                    </div>
                  </div>
                );
             })()
             ...
             <div className="space-y-6 pb-20">
                <div className="space-y-3">
                    <div className="relative">
                      <input type="text" autoFocus placeholder={userLocation ? "Buscar lugares a tu alrededor (30km)..." : "Buscar en la app..."} className="w-full p-4 pl-12 rounded-xl shadow-sm border border-stone-200 outline-none focus:ring-2 focus:ring-cyan-500" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') handleQuickSearch(searchQuery); }} />
                      <Search className="absolute left-4 top-4 text-stone-400" size={20} />
                      {searchQuery && <button onClick={() => handleQuickSearch(searchQuery)} className="absolute right-3 top-2.5 bg-cyan-600 text-white p-1.5 rounded-lg text-xs font-bold shadow-sm hover:bg-cyan-700 transition-colors"> Buscar </button>}
                    </div>
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                        <button onClick={() => handleQuickSearch('Hoteles')} className="flex items-center gap-1.5 px-3 py-2 bg-white border border-stone-200 rounded-full text-xs font-bold text-stone-600 hover:bg-cyan-50 hover:text-cyan-700 hover:border-cyan-200 transition-colors whitespace-nowrap shadow-sm"> <Hotel size={14} className="text-indigo-500"/> Hoteles </button>
                        <button onClick={() => handleQuickSearch('Restaurantes')} className="flex items-center gap-1.5 px-3 py-2 bg-white border border-stone-200 rounded-full text-xs font-bold text-stone-600 hover:bg-cyan-50 hover:text-cyan-700 hover:border-cyan-200 transition-colors whitespace-nowrap shadow-sm"> <Utensils size={14} className="text-orange-500"/> Restaurantes </button>
                        <button onClick={() => handleQuickSearch('Hospitales')} className="flex items-center gap-1.5 px-3 py-2 bg-white border border-stone-200 rounded-full text-xs font-bold text-stone-600 hover:bg-cyan-50 hover:text-cyan-700 hover:border-cyan-200 transition-colors whitespace-nowrap shadow-sm"> <Stethoscope size={14} className="text-red-500"/> Salud </button>
                        <button onClick={() => handleQuickSearch('Tiendas')} className="flex items-center gap-1.5 px-3 py-2 bg-white border border-stone-200 rounded-full text-xs font-bold text-stone-600 hover:bg-cyan-50 hover:text-cyan-700 hover:border-cyan-200 transition-colors whitespace-nowrap shadow-sm"> <ShoppingBag size={14} className="text-purple-500"/> Tiendas </button>
                        <button onClick={() => handleQuickSearch('Farmacias')} className="flex items-center gap-1.5 px-3 py-2 bg-white border border-stone-200 rounded-full text-xs font-bold text-stone-600 hover:bg-cyan-50 hover:text-cyan-700 hover:border-cyan-200 transition-colors whitespace-nowrap shadow-sm"> <Plus size={14} className="text-green-500"/> Farmacias </button>
                    </div>
                </div>
                {searchQuery ? (
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                     {isSearchingExternal ? <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 text-center"> <div className="animate-spin text-cyan-600 mx-auto mb-2 w-fit"><Compass size={24}/></div> <p className="text-stone-500 text-sm font-bold">Buscando "{searchQuery}" cerca de ti...</p> </div> : externalSearchResults.length > 0 && <div> <div className="flex items-center justify-between mb-3 px-1"> <h3 className="font-bold text-stone-600 text-sm uppercase flex items-center gap-2"> <MapPin size={16} className="text-emerald-600"/> Resultados Cercanos (30km) </h3> <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-bold">Radar Local</span> </div> <div className="grid gap-3"> {externalSearchResults.map((place, idx) => <div key={idx} className="bg-white p-4 rounded-xl border border-stone-100 hover:border-emerald-300 shadow-sm transition-all group relative"> <div className="flex justify-between items-start"> <div> <h4 className="font-bold text-stone-800 text-sm group-hover:text-emerald-700">{place.name}</h4> <p className="text-xs text-stone-500 mt-1">{place.address}</p> <div className="flex items-center gap-2 mt-2"> {place.isOpen && <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-bold">ABIERTO</span>} {place.rating && <span className="text-[10px] flex items-center gap-0.5 font-bold text-amber-500"><Award size={10}/> {place.rating}</span>} </div> </div> <a href={place.mapLink} target="_blank" rel="noopener noreferrer" className="bg-stone-100 hover:bg-emerald-600 hover:text-white text-stone-600 p-2 rounded-full transition-colors"> <Navigation size={16} /> </a> </div> </div>)} </div> </div>}
                     {filteredUsers.length > 0 && <div> <h3 className="font-bold text-stone-600 text-sm uppercase mb-3 px-1">Personas</h3> <div className="flex overflow-x-auto gap-3 pb-2 no-scrollbar"> {filteredUsers.map(u => <div key={u.id} onClick={() => handleUserClick(u.id)} className="min-w-[120px] bg-white p-3 rounded-xl border border-stone-100 flex flex-col items-center cursor-pointer hover:border-cyan-300 shadow-sm transition-all"> <img src={u.avatar} className="w-12 h-12 rounded-full object-cover mb-2" alt={u.name} /> <span className="text-xs font-bold text-stone-800 text-center line-clamp-1">{u.name}</span> <span className="text-[10px] text-stone-400 text-center">Ver Perfil</span> </div>)} </div> </div>}
                     {searchGroups.length > 0 && <div> <div className="flex items-center justify-between mb-3 px-1"> <h3 className="font-bold text-stone-600 text-sm uppercase">Grupos de Viaje</h3> <span className="text-xs bg-stone-100 px-2 py-1 rounded-full text-stone-500">{searchGroups.length}</span> </div> <div className="grid gap-3"> {searchGroups.map(group => <div key={group.id} onClick={() => handleOpenGroup(group.id)} className="bg-white p-3 rounded-xl border border-stone-100 flex items-center space-x-3 shadow-sm cursor-pointer hover:border-cyan-300 transition-colors"> <img src={group.imageUrl} className="w-12 h-12 rounded-lg object-cover" alt={group.name}/> <div className="flex-1"> <div className="flex justify-between items-start"> <h4 className="font-bold text-stone-800 text-sm">{group.name}</h4> {group.isPrivate && <Lock size={12} className="text-stone-400"/>} </div> <p className="text-xs text-stone-500 line-clamp-1">{group.description}</p> </div> </div>)} </div> </div>}
                     {searchDestinations.length > 0 && <div> <div className="flex items-center justify-between mb-3 px-1"> <h3 className="font-bold text-stone-600 text-sm uppercase">Destinos</h3> <span className="text-xs bg-stone-100 px-2 py-1 rounded-full text-stone-500">{searchDestinations.length}</span> </div> <div className="grid gap-3"> {searchDestinations.slice(0, 5).map(dest => <div key={dest.id} onClick={() => handleOpenGuide(dest.name)} className="bg-white p-3 rounded-xl border border-stone-100 flex items-center space-x-3 shadow-sm cursor-pointer hover:border-cyan-300 transition-colors"> <img src={dest.imageUrl} className="w-12 h-12 rounded-lg object-cover" alt={dest.name}/> <div className="flex-1"> <h4 className="font-bold text-stone-800 text-sm">{dest.name}</h4> <div className="flex items-center text-xs text-stone-500 mt-0.5"> <MapPin size={10} className="mr-1" /> {dest.location} </div> </div> </div>)} </div> </div>}
                     {filteredPosts.length > 0 && <div> <div className="flex items-center justify-between mb-3 px-1"> <h3 className="font-bold text-stone-600 text-sm uppercase">Publicaciones</h3> <span className="text-xs bg-stone-100 px-2 py-1 rounded-full text-stone-500">{filteredPosts.length}</span> </div> <div className="grid grid-cols-3 gap-1"> {filteredPosts.map(post => <div key={post.id} className="aspect-square relative cursor-pointer bg-gray-100 rounded-lg overflow-hidden" onClick={() => handleViewPost(post)}> {post.mediaType === 'video' ? <video src={post.imageUrl} className="w-full h-full object-cover" /> : <img src={post.imageUrl} className="w-full h-full object-cover" alt="" />} </div>)} </div> </div>}
                  </div>
      ...
      <div className="fixed bottom-0 w-full bg-white border-t border-stone-200 flex justify-around items-center p-3 md:hidden z-50 pb-safe">
        <button onClick={() => navigateToTab('home')} className={`flex flex-col items-center gap-0.5 transition-colors ${activeTab === 'home' ? 'text-cyan-600' : 'text-stone-400'}`}> <Home size={24} strokeWidth={activeTab === 'home' ? 2.5 : 2} /> <span className="text-[10px] font-medium">Inicio</span> </button>
        <button onClick={() => navigateToTab('explore')} className={`flex flex-col items-center gap-0.5 transition-colors ${activeTab === 'explore' ? 'text-cyan-600' : 'text-stone-400'}`}> <Compass size={24} strokeWidth={activeTab === 'explore' ? 2.5 : 2} /> <span className="text-[10px] font-medium">Explorar</span> </button>
        <button onClick={() => openModal(setIsCreateModalOpen)} className="relative -top-5 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-full p-4 shadow-lg shadow-cyan-200 border-4 border-stone-50 active:scale-95 transition-transform"> <Plus size={28} strokeWidth={3} /> </button>
        <button onClick={() => navigateToTab('messages')} className={`flex flex-col items-center gap-0.5 transition-colors relative ${activeTab === 'messages' ? 'text-cyan-600' : 'text-stone-400'}`}> <MessageCircle size={24} strokeWidth={activeTab === 'messages' ? 2.5 : 2} /> {unreadMessagesCount > 0 && <span className="absolute top-0 right-0 bg-red-500 w-2 h-2 rounded-full"></span>} <span className="text-[10px] font-medium">Mensajes</span> </button>
        <button onClick={() => { navigateToTab('profile'); }} className={`flex flex-col items-center gap-0.5 transition-colors ${activeTab === 'profile' ? 'text-cyan-600' : 'text-stone-400'}`}> <div className={`relative rounded-full overflow-hidden w-7 h-7 ring-2 transition-all ${activeTab === 'profile' && !viewingProfileId ? 'ring-cyan-600 ring-offset-2' : 'ring-transparent'}`}> <img src={user.avatar} alt="Perfil" className="w-full h-full object-cover" /> </div> <span className="text-[10px] font-medium">Perfil</span> </button>
      </div>
