...
export default function App() {
  ...
  const performNearbySearch = async (query: string) => {
      ...
      // para garantizar que los resultados sean realmente "cercanos".
      if (!userLocation) {
          setExternalSearchResults([]);
          setIsSearchingExternal(false);
          // Opcional: Podríamos mostrar una alerta aquí si quisiéramos ser más explícitos
          return;
      }
                         ...
                         <div>
                             <div className="flex items-center justify-between mb-3 px-1">
                                 <h3 className="font-bold text-stone-600 text-sm uppercase flex items-center gap-2">
                                     <MapPin size={16} className="text-emerald-600"/> Resultados Cercanos (30km)
                                 </h3>
                                 <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-bold">Radar Local</span>
                             </div>
